    ...
    private fun updateDepartures(showRefreshingIndicator: Boolean = true) {
        coroutineScope.launch {
            ...
            when (stopMonitorInfoResource) {
                is Result.Error -> {
                    _sideEffect.send(DeparturesScreenSideEffect.ShowNetworkError(stopMonitorInfoResource.error))
    ...
    private suspend fun fetchStopSchedule(
        ...
        return if (departure.stopSchedule != null) {
            ...
            when (stopScheduleResource) {
                is Result.Error -> {
                    _sideEffect.send(DeparturesScreenSideEffect.ShowNetworkError(stopScheduleResource.error))
                    null
                }
